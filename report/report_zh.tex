...
最终数据集包含 \textbf{1,037,826} 对五言对句。数据随机划分为训练集（80\%）、验证集（10\%）和测试集（10\%）。
...
表~\ref{tab:results} 展示了主要定量结果。预训练GPT-2模型在困惑度上显著优于两个从零实现模型（1.49 对比 79.15 和 102.44），这符合预期——其容量更大（102M vs. 11M参数）且在相关诗歌数据上进行了大量预训练。BLEU-4分数遵循相同的排序，但所有模型的绝对值都较低——这是创造性文本生成中的已知现象，因为给定提示存在许多有效的续写方式~\cite{novikova2017why}。

在两个从零实现模型中，Decoder-Only架构取得了更好的困惑度（79.15 vs. 102.44）。这有些出乎意料，因为Encoder-Decoder参数量略多且具有双向编码能力。我们将此归因于两个因素：(1) Decoder-Only模型将上句作为自回归上下文的一部分，而Encoder-Decoder需要通过交叉注意力来学习关注上句；(2) 默认超参数（层数）可能未针对Encoder-Decoder进行最优平衡。
...
表~\ref{tab:examples} 展示了各模型在经典上句上的定性生成结果。由于模型参数量有限（11M--12M），从零实现模型无法复现原诗的标准下句，但生成了在语义和意境上具有一定合理性的变体。预训练GPT-2模型则生成了格律更规整、对仗更工整的输出，体现出大规模预训练的优势。

\subsection{训练动态}

从零实现模型在30个训练轮次中逐步收敛。预训练模型因强大的语言模型先验知识收敛更快（10轮）。
...
\begin{enumerate}[nosep]
    \item 适度容量的从零实现Transformer（11M--12M参数）能够学习有意义的诗歌生成能力，取得合理的困惑度并生成语义连贯的下句。
    \item Decoder-Only架构在我们的设置中优于Encoder-Decoder（PPL 79.15 vs. 102.44），可能因为更有效地利用了上句上下文，以及更优的容量分配。
    \item 微调领域特定的预训练模型（uer/gpt2-chinese-poem）取得了显著优越的结果（PPL 1.49 vs. 79--102），证明了大规模预训练即使在专业文学领域也具有巨大价值。
\end{enumerate}
